...
def create_cluster(redshift, roleArn, DWH_CLUSTER_TYPE, DWH_NODE_TYPE, DWH_NUM_NODES, DWH_DB, DWH_CLUSTER_IDENTIFIER,
    ...
    try:
        ...
        if e.response['Error']['Code']:
            print('Redshift cluster already exist!')
        else:
            print(e)
...
def get_cluster_props(redshift, DWH_CLUSTER_IDENTIFIER):
    ...
    if myClusterProps['ClusterStatus'] == 'creating':
        print('Cluster creation is still in progress... Try again later!!!')
        return None
...
def main(argv):
    ...
    if not argv[1:]:
        print("In order to run properly you have to specify 'create' or 'delete' as an argument")
        exit()
    elif argv[1] == "create":
